...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('body').append("<h2> append()사용추가 </h2>");
		// 본문 앞쪽 추가 
		$('body').prepend("<h2> prepend()사용추가 </h2>")
		...
		// each() / $(요소선택자).each()
		// => 선택한 여러 개의 요소를 순차적으로 접근할 때 사용 
		
		// 배열의 값을 각각 접근해서 테이블 행에 추가 
		// 테이블 태그에 배열의 값을 추가 - append()
		
	 	$.each(arr,function(index,item){ // 선택자로 접근이 안될때
			// index - 위치, item - 요소의 값
			//alert("확인");
			//alert("index : " +index + "/ item : " + item.name); //객체.변수
